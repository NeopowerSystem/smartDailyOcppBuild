...
var getWaitingQueue_exports = {};
__export(getWaitingQueue_exports, {
  default: () => getWaitingQueue_default
});
module.exports = __toCommonJS(getWaitingQueue_exports);
...
function getChargingParameter() {
  const stmt = db2.prepare(`SELECT * FROM charging_parameter`);
  return stmt.all();
}
...
function getAllSmartMeterVendorId() {
  const stmt = db3.prepare(`SELECT DISTINCT vendorId FROM smartMeters`);
