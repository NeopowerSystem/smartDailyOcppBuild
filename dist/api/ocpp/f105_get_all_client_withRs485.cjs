...
var f105_get_all_client_withRs485_exports = {};
__export(f105_get_all_client_withRs485_exports, {
  default: () => f105_get_all_client_withRs485_default
});
module.exports = __toCommonJS(f105_get_all_client_withRs485_exports);
  ...
  try {
    ...
    if (chargers.length) {
      ...
      formattedChargers = chargers.reduce((acc, row) => {
        ...
        if (connectorId) {
          const statusCode = STATUS_CODE_MAP2[connectorStatus] ?? -1;
          const connectorInfo = {
            connectorId,
    ...
    const formattedRs485 = rs485Raw.map((data) => {
      const connectors = data.connectors && data.connectors.length > 0 ? data.connectors.map((connector) => {
        ...
        if (connector.status != null) formattedConnector.status_code = STATUS_CODE_MAP2[connector.status] ?? -1;
