...
var f003_remote_stop_charging_exports = {};
__export(f003_remote_stop_charging_exports, {
  default: () => f003_remote_stop_charging_default
});
module.exports = __toCommonJS(f003_remote_stop_charging_exports);
...
function loadIdTags() {
  try {
    const stmt = db4.prepare("SELECT * FROM idTags");
    const rows = stmt.all();
    return rows;
  } catch (err) {
...
function findIdTag(idTag) {
  try {
    const stmt = db4.prepare("SELECT * FROM idTags WHERE idTag = ?");
...
function findIdTagByChargerId(chargerId) {
  try {
    const stmt = db4.prepare("SELECT * FROM idTags WHERE chargerId = ? LIMIT 1");
...
function deleteIdTag(idTag) {
  try {
    const stmt = db4.prepare("DELETE FROM idTags WHERE idTag = ?");
    stmt.run(idTag);
  } catch (err) {
...
function getChargingParameter() {
  const stmt = db5.prepare(`SELECT * FROM charging_parameter`);
  return stmt.all();
}
...
function getOCPPPool() {
  const stmt = db6.prepare(`SELECT * FROM OCPPPool`);
  return stmt.all();
}
...
function getSmartMeterPool() {
  const stmt = db7.prepare(`SELECT * FROM SmartMeterPool`);
  return stmt.all();
}
...
async function sendChargingInfo({ DeviceId, DeviceTime, ConnectorId, TransactionId, Voltage, Current, WH }) {
  ...
  try {
    const response2 = await import_axios3.default.put(url, body, {
      headers: {
        "Authorization": "Bearer zaxb20C3jlV6dX8Dqvy2CpOvcnU7oqsK6mE4HigH",
        "Content-Type": "application/json"
      }
    ...
    try {
      ...
      if (removeRes.status === "success") {
        ...
        const StatusChange_body = {
          ...
          "ConnectorId": "1",
          "Status": STATUS_CODE_MAP2["Preparing"].toString(),
          "ErrorCode": null,
          "VendorErrorCode": null
        };
        sendStatusChange(StatusChange_body);
