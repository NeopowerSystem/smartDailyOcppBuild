...
var f001_get_charger_info_exports = {};
__export(f001_get_charger_info_exports, {
  default: () => f001_get_charger_info_default
});
module.exports = __toCommonJS(f001_get_charger_info_exports);
...
async function getMaxPower() {
  const stmt = db2.prepare(`SELECT * FROM maxPower`);
  return stmt.all();
}
...
function generateTransactionId(chargerId, connectorId) {
  try {
    const lastTransactionStmt = db3.prepare(`SELECT MAX(transactionId) AS transactionId FROM transactions`);
    ...
    const transactionStmt = db3.prepare(`
    ...
    const connectorStmt = db3.prepare(`
            UPDATE connectors
            SET transactionId = ?
...
function getSmartMeterPool() {
  const stmt = db4.prepare(`SELECT * FROM SmartMeterPool`);
  return stmt.all();
}
...
function loadIdTags() {
  try {
    const stmt = db5.prepare("SELECT * FROM idTags");
    const rows = stmt.all();
    return rows;
  } catch (err) {
...
function findIdTag(idTag) {
  try {
    const stmt = db5.prepare("SELECT * FROM idTags WHERE idTag = ?");
...
function findIdTagByChargerId(chargerId) {
  try {
    const stmt = db5.prepare("SELECT * FROM idTags WHERE chargerId = ? LIMIT 1");
...
function deleteIdTag(idTag) {
  try {
    const stmt = db5.prepare("DELETE FROM idTags WHERE idTag = ?");
    stmt.run(idTag);
  } catch (err) {
